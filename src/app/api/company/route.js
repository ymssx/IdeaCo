...
export async function POST(request) {
  ...
  try {
    const { companyName, bossName, secretaryConfig, language } = await request.json();
    if (!companyName) {
    ...
    // Set company language if provided during creation (default: 'en')
    if (language) {
      try { company.setLanguage(language); } catch { /* ignore invalid */ }
    }
...
export async function PUT(request) {
  ...
  try {
    ...
    // Support language change via PUT
    if (body.language) {
      try {
        company.setLanguage(body.language);
      } catch (e) {
